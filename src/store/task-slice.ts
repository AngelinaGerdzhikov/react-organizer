  ...
  reducers: {
    ...
    removeTask(state, action) {
      ...
      if (updatedTaskIdsPerDate[action.payload.dateKey]) {
        updatedTaskIdsPerDate[action.payload.dateKey] = updatedTaskIdsPerDate[
          action.payload.dateKey
        ].filter((taskId) => taskId !== action.payload.id);
      }
      return {
        ...state,
        tasks: updatedTasks,
        taskHasBeenDeleted: true,
        taskIdsPerDate: updatedTaskIdsPerDate,
      };
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTaskAsync.fulfilled, (state, action) => {
        state.status = "idle";
        const taskIndex = state.newTasks.findIndex(
          (task) => task.id === action.payload.id
        );
        console.log(taskIndex);
        state.newTasks[taskIndex] = action.payload;
        console.log(
          `Updated task: ${JSON.stringify(state.newTasks[taskIndex])}`
        );
      })
